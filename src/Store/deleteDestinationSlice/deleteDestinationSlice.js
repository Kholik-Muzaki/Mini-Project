...
export const fetchDeleteDestination = createAsyncThunk("fetch/deleteDestination", APIDestination.deleteDestination
);
...
const deleteDestinationSlice = createSlice({
    ...
    extraReducers: (builder) => {
      builder.addCase(fetchDeleteDestination.fulfilled, (state, { payload }) => {
        ...
        state.data = state.data.filter((destination) => destination.id !== payload.id);
      });
