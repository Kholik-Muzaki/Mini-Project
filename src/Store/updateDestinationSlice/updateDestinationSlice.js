...
export const fetchUpdateDestination = createAsyncThunk("destination/update", async (payload) => {
    try {
        const updatedData = await APIDestination.updateDestination(payload.id, payload.data);
        return updatedData;
    } catch (error) {
        throw error;
    }
